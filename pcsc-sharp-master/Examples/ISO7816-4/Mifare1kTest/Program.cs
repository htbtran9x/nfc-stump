        ...
        public static void Main() {
            using (var context = ContextFactory.Instance.Establish(SCardScope.System)) {
                ...
                using (var isoReader = new IsoReader(
                    ...
                    Console.WriteLine(string.Concat(uid));
                    Console.WriteLine("UID: {0}",
                        (uid != null)
                            ? BitConverter.ToString(uid)
                            : throw new Exception("GET DATA failed."));

                    var sscid = card.ReadBinary(0x00, 0x04, 10);
                    Console.WriteLine("SSC: {0}",
                      (sscid != null)
                          ? BitConverter.ToString(sscid)
                          : throw new Exception("GET SSC failed."));
